...
const Plan = () => {
  ...
  const componentToRender = () => {
    if (curr === 0) {
          ...
          <div className={"plansContainer"}>
            {planData.map((plan, idx) => (
              <PlanCard
                ...
                userPlan={state.loggedUser.organization?.name}
                userPlanRank={state.loggedUser.organization?.rank}
              />
            ))}
          </div>
      ...
      return <div className={"parentContainer"}>Bills</div>;
    }
  };
  ...
  return (
    <div className={"planPage"}>
      <div className={"accountNav"}>
        <div
          className={`${
            curr === 0 ? "selectedItem" : "unselectedItem"
          } navItem`}
          onClick={() => setCurr(0)}
        >
          <span className={"itemText"}>Plans</span>
        </div>
        <div
          className={`${
            curr === 1 ? "selectedItem" : "unselectedItem"
          } navItem`}
          onClick={() => setCurr(1)}
        >
          <span className={"itemText"}>Bills</span>
        </div>
      </div>
      {componentToRender()}
    </div>
